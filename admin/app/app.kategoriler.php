...
if($_GET["p"]=="mekan_edit"){
	$cat_id 			= p("kategori_id");
	$kategori_adi		= p("kategori_adi");
	$kategori_url		= $kategori_adi;
	$kategori_desc 		= p("kategori_desc");
	$en_kategori_adi		= p("en_kategori_adi");
	$en_kategori_url		= $en_kategori_adi;
	$en_kategori_desc 		= p("en_kategori_desc");
	...
	if(!$kategori_adi ){
		...
		$update =$db->query("UPDATE salonlar SET
			kategori_ust_id 	= '$ust_kategori',
			kategori_adi 		= '$kategori_adi',
			kategori_desc 		= '$kategori_desc',
			kategori_url		= '$kategori_url',
...
if($_GET["p"]=="mekan_add"){
	...
	if(!$kategori_adi ){
		...
		$insert =$db->query("INSERT INTO salonlar SET
			kategori_ust_id 	= '$ust_kategori',
			kategori_adi 		= '$kategori_adi',
			kategori_url		= '$kategori_url',
			kategori_desc 		= '$kategori_desc',
...
if($_GET["p"]=="tek_mekan_sil"){
	$id = p("id");
	$kontrol = $db->query("SELECT * FROM salonlar WHERE kategori_id='$id'");
	if($kontrol->rowCount()){
		$kategori = $kontrol->fetch(PDO::FETCH_ASSOC);
		$delete = $db->query("DELETE FROM salonlar WHERE kategori_id='$id'");
		if($delete->rowCount()){
			echo 'basarili';
		}else{
			echo 'basarisiz';
...
if($_GET["p"]=="mekan_durum_degis"){
	...
	if($kontrol->rowCount()){
		...
		$uyeDurum = $uyeRow["kategori_durum"];
		if($uyeDurum==1){
			$update = $db->query("UPDATE salonlar SET kategori_durum=0 WHERE kategori_id='$id'");
			if($update->rowCount()){
				echo 'yasaklama-basarili';
			}
		}else{
			$update = $db->query("UPDATE salonlar SET kategori_durum=1 WHERE kategori_id='$id'");
			if($update->rowCount()){
				echo 'yasak-kaldirildi';
			}
		}
