...
if($_GET["p"]=="koltuk_edit"){
	...
	if(!$urun_adi || !$urun_id){
		...
		$update =$db->query("UPDATE koltuklar SET
			urun_adi 		= '$urun_adi',
			urun_url 		= '$urun_url',
			urun_kategori 	= '$urun_kategori',
			urun_icerik 	= '$urun_desc',
...
if($_GET["p"]=="koltuk_add"){
	...
	if(!$urun_adi ){
		...
		$siraQuery = $db->query("SELECT urun_sira_no FROM koltuklar WHERE urun_kategori='$urun_kategori' ORDER BY urun_sira_no DESC LIMIT 0,1")->fetch(PDO::FETCH_ASSOC);
		$newSira 	= $siraQuery["urun_sira_no"]+1;
		$insert = $db->prepare("INSERT INTO koltuklar SET 
			urun_adi=:urun_adi,
			urun_sira_no=:sira_no,
			urun_url=:url,
			urun_icerik=:icerik,
...
#Tek Ürün Sil 
if($_GET["p"]=="tek_koltuk_sil"){
	$id = p("id");
	$kontrol = $db->query("SELECT * FROM koltuklar WHERE urun_id='$id'");
	if($kontrol->rowCount()){
		$urun = $kontrol->fetch(PDO::FETCH_ASSOC);
		$delete = $db->query("DELETE FROM koltuklar WHERE urun_id='$id'");
		if($delete->rowCount()){
			echo 'basarili';
		}else{
			echo 'basarisiz';
...
if($_GET["p"]=="koltuk_durum_degis"){
	...
	if($kontrol->rowCount()){
		...
		$urunDurum = $uyeRow["urun_durum"];
		if($urunDurum==1){
			$update = $db->query("UPDATE koltuklar SET urun_durum=0 WHERE urun_id='$id'");
			if($update->rowCount()){
				echo 'yasaklama-basarili';
			}
		}else{
			$update = $db->query("UPDATE koltuklar SET urun_durum=1 WHERE urun_id='$id'");
			if($update->rowCount()){
				echo 'yasak-kaldirildi';
			}
		}
